/* You need to make a directory named "quorum" where this file placed.
   And the three functions called in main represent the situation when there are 2 nodes, 1000
   nodes, and a star topology?where we only consider the neighbors of node0 in the map, adjust
   the duty cycle of node 0 with two algorithms?and output the relationship between duty cycle,
   energy percentage and latency of node 0 and time.
...
void twoNodeNoModCollision(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
  			if(A.quorum_isOff(timeFrame) == false && B.quorum_isOff(timeFrame) == false){//i and j are neighbors in default
    ...
    stringstream ss;
    ss << "quorum/two_node_latency.txt";
    fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
    for (int i=0;i<21;i++)
    {
...
//			t = Seconds(timeFrame);
		...
		for(int i = 0; i < 1000; i++){
			if(warray[i].quorum_isOff(timeFrame) == false){
				for(int j = 0; j < 1000; j++){
					if(warray[j].quorum_isOff(timeFrame) == false && neighborMap[i][j] == 1){
						onNeighbor[j]++;
					}
				}
			}
		...
		for(int i = 0; i < 1000; i++){
			if(onNeighbor[i] == 1 && warray[i].quorum_isOff(timeFrame) ==false){
				for(int j = 0; j < 1000; j++){
					if(neighborMap[i][j] == 1 && onNeighbor[j] == 1 && warray[j].quorum_isOff(timeFrame) ==false){
	...
	stringstream ss;
	ss << "quorum/thousand_node_latency.txt";
	fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
...
//
	stringstream ss;
	ss << "quorum/thousand_times_latency.txt";
	fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
...
//			for(int i = 0; i < 1000; i++){
//				if(warray[i].alive == true && warray[i].quorum_isOff(timeFrame) == false){
//					warray[i].energy -= pn;
...
//					}
//					for(int j = 0; j < 1000; j++){
//						if(warray[j].alive == true && warray[j].quorum_isOff(timeFrame) == false && neighborMap[i][j] == 1){
//							onNeighbor[j]++;
//						}
//					}
//				}
//			}
//			for(int i = 0; i < 1000; i++){
//				if(onNeighbor[i] == 1 && warray[i].alive == true && warray[i].quorum_isOff(timeFrame) ==false){
//					for(int j = 0; j < 1000; j++){
//						if(neighborMap[i][j] == 1 && onNeighbor[j] == 1 && warray[j].alive == true && warray[j].quorum_isOff(timeFrame) ==false){
//							setLatency(i, j, timeFrame);
//							cout<<timeFrame<<" "<<i<<" "<<j<<endl;
//						}
//					}
...
//		}
//		stringstream ss;
//    	ss << "quorum/energy_node_latency.csv";
//    	fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
//
//		for(int i = 0; i < 1000; i++){
//			for(int j = 0; j < 1000; j++){
...
//	warray[0].energy = Pmax;
	...
	for(int k = 0; k < CYCLENUM && warray[i].alive == true; k++){
		for(int timeFrame = warray[i].startTime; timeFrame <= timeInterval && warray[i].alive == true; timeFrame ++){
			if(warray[i].alive == true && warray[i].quorum_isOff(timeFrame) == false){
				warray[i].energy -= pn;
...
//				}
				...
				if(onNeighbor == 1){
					for(int j = 0; j < 1000; j++){
						if(neighborMap[i][j] == 1 && warray[j].alive == true && warray[j].quorum_isOff(timeFrame) == false && findNeighborTime[j] == 0){
...
//	warray[0].energy = Pmax;
	...
	for(int k = 0; k < CYCLENUM && warray[i].alive == true; k++){
		for(int timeFrame = 1; timeFrame <= timeInterval && warray[i].alive == true; timeFrame ++){
			if(warray[i].alive == true && warray[i].quorum_isOff(timeFrame) ==false){
				warray[i].energy -= pn;
//				if(warray[i].energy <= Pmin && flag == true){
//					flag == false;
...
void twoNodeNoModCollisionSymmetric(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
    ...
    stringstream ss;
    ss << "quorum/two_node_latency_sym.txt";
    fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
    for (int i=0;i<21;i++)
    {
      double th = i*0.01 + 0.1;
