...
{
	stringstream ss;
	ss << "hello/energy_theta_table.txt";
...
void twoNodeNoModCollision(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
  			if(A.hello_isOff(timeFrame) == false && B.hello_isOff(timeFrame) == false){//i and j are neighbors in default
...
//
	stringstream ss;
	ss << "hello/thousand_times_latency.txt";
	fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
...
//			t = Seconds(timeFrame);
			...
			for(int i = 0; i < 1000; i++){
				if(warray[i].hello_isOff(timeFrame) == false){
					for(int j = 0; j < 1000; j++){
						if(warray[j].hello_isOff(timeFrame) == false && neighborMap[i][j] == 1){
							onNeighbor[j]++;
						}
					}
				}
			...
			for(int i = 0; i < 1000; i++){
				if(onNeighbor[i] == 1 && warray[i].hello_isOff(timeFrame) ==false){
					for(int j = 0; j < 1000; j++){
						if(neighborMap[i][j] == 1 && onNeighbor[j] == 1 && warray[j].hello_isOff(timeFrame) ==false){
							setLatency(i, j, timeFrame);
							cout<<timeFrame<<" "<<i<<" "<<j<<endl;
						}
					}
...
//	warray[0].energy = Pmax;
	...
	for(int k = 0; k < CYCLENUM && warray[i].alive == true; k++){
		for(int timeFrame = warray[0].startTime; timeFrame <= timeInterval && warray[i].alive == true; timeFrame ++){
			if(warray[i].alive == true && warray[i].hello_isOff(timeFrame) == false){
				warray[i].energy -= pn;
...
//				}
				...
				for(int j = 0; j < 1000; j++){
					if(warray[j].alive == true && warray[j].hello_isOff(timeFrame) == false && neighborMap[i][j] == 1){
						onNeighbor++;
					}
				}
				if(onNeighbor == 1){
					for(int j = 0; j < 1000; j++){
						if(neighborMap[i][j] == 1 && warray[j].alive == true && warray[j].hello_isOff(timeFrame) == false && findNeighborTime[j] == 0){
...
void twoNodeNoModCollisionSymmetric(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
    ...
    stringstream ss;
    ss << "hello/two_node_latency_sym.txt";
    fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
    for (int i=0;i<21;i++)
    {
      double th = i*0.01 + 0.1;
