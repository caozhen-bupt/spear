/* You need to make a directory named "hedis" where this file placed.
...
void twoNodeNoModCollision(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
  			if(A.hedis_isOff(timeFrame) == false && B.hedis_isOff(timeFrame) == false){//i and j are neighbors in default
...
//				}
				...
				if(onNeighbor == 1){
					for(int j = 0; j < 1000; j++){
						if(neighborMap[i][j] == 1 && warray[j].alive == true && warray[j].hedis_isOff(timeFrame) == false && findNeighborTime[j] == 0){
...
void twoNodeNoModCollisionSymmetric(int timeInterval, int arrayLength)
{
  ...
  //				t = Seconds(timeFrame);
    ...
    stringstream ss;
    ss << "hedis/two_node_latency_sym.txt";
    fstream f (ss.str().c_str(), std::ios::out | std::ios::app);
    for (int i=0;i<21;i++)
    {
      double th = i*0.01 + 0.1;
