...
public:
	...
	bool hello_isOff_collision1(int time){
		int num = rand()%10;
		if(hello_isOff(time) == false){	
			if(num*1.0/10 < PROB1){
				return false;
			}
    	}
	...
	bool hello_isOff_collision2(int tt, int timeInterval){
	...
	for(double prob = 0.1; prob <= 0.9; prob+=0.1){
		...
		for(int s = 0; s < TIMES; s++){
			for(int timeFrame = w[0].startTime; timeFrame <= timeInterval; timeFrame ++){
				if(w[0].hello_isOff_collision1(timeFrame) == false){
					for(int j = 1; j < NODENUM; j++){
						if(w[j].hello_isOff_collision1(timeFrame) == false){
							onNeighbor++;
						}
					}
					if(onNeighbor == 1){
						for(int j = 1; j < NODENUM; j++){
							if(w[j].hello_isOff_collision1(timeFrame) == false && findNeighborTime[j] == 0){
...
//					temp[x] = w[x].hello_isOff_collision2(timeFrame, timeInterval);
//				}
//				if(temp[0] == false){
//					for(int j = 1; j < NODENUM; j++){
//						if(temp[j] == false){
...
void ModCollision_2(int timeInterval)
{
	...
	for(double prob = 0.1; prob <= 0.9; prob+=0.1){
		...
		f<<w[0].PROB2<<","<<sum*1.0/NODENUM/TIMES<<endl;
		sum=0;
