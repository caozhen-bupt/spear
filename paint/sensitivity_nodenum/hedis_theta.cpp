			...
			for(int timeFrame = w[0].startTime; timeFrame <= timeInterval; timeFrame ++){
				if(w[0].hedis_isOff_collision1(timeFrame) == false){
					for(int j = 1; j < NODENUM; j++){
						if(w[j].hedis_isOff_collision1(timeFrame) == false){
							onNeighbor++;
						}
					}
					if(onNeighbor == 1){
						for(int j = 1; j < NODENUM; j++){
							if(w[j].hedis_isOff_collision1(timeFrame) == false && findNeighborTime[j] == 0){
