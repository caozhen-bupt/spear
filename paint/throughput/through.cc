...
const double timeFrame=10000;
int latency0[NODENUM][NODENUM];
int latency1[NODENUM][NODENUM];
int latency2[NODENUM][NODENUM];
int neighborMap[NODENUM][NODENUM];
std::vector<NodeContainer> nConnect; 
std::vector<NetDeviceContainer> deviceContainer;
int loadMatrix()
{
	
}
void createTop(int t, NodeContainer c, PointToPointHelper p, Ipv4AddressHelper ip)
{
	for(int i=0; i<NODENUM; i++){
		for(int j=0; j<NODENUM; j++){
			if(latency0[i][j] == t){
				...
				NodeContainer nc = NodeContainer (c.Get (i), c.Get (j));
				NetDeviceContainer dc = p.install(nc);
				nConnect.add(nc);
				deviceContainer.add(dc);
				ip.SetBase ("10.1."+istr+"."+jstr, "255.255.0.0");
  				ip.Assign (dc);
